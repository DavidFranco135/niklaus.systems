...
const getAI = () => {
  if (!aiInstance) {
    const apiKey = "AIzaSyC0f2TqTaAQYZOJBtHZq7_kG3YH5pHOwHU";
...
export const suggestPromotions = async (salesData: any) => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Com base nestes dados de vendas: ${JSON.stringify(
        salesData
      )}, sugira 3 promoções para aumentar o faturamento.`,
    });
